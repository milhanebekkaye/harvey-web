...
9. **Auto-refresh after chat tools**: When any message in the **current turn** (since the user's last send) includes a tool call (e.g. add task, modify schedule, delete task, regenerate schedule), `ProjectChatView` calls `onTasksChanged`, which triggers `fetchTasks()`. The component sets a ref to the message count at **send time** (in `handleSubmit` before `sendMessage`); in `onFinish` it slices only messages added this turn and checks for an assistant message with a tool call. That way multi-step responses (e.g. tool call then narration) still trigger one refresh, and plain-text follow-up replies do not trigger a refetch.
...
- **Persistence**: TimelineView calls `onReorder(...)` with the computed payload; `handleReorder` in `dashboard/page.tsx` applies an **optimistic update** to `tasks` state immediately (moving the task to the correct section, updating `scheduledDate`/`isFlexible`/`windowStart`/`windowEnd`/`position`, and re-sorting affected sections), then fires `POST /api/tasks/reorder` in the background. On success, `fetchTasks({ silent: true })` syncs with the server without showing a loading spinner. On failure, `tasks` is reverted to the pre-drop snapshot and an error toast is shown.
