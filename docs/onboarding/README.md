...
- `src/app/onboarding/welcome/page.tsx`
  - Welcome screen for new users with no name. Same visual style as signin (aurora background, glass card). Avatar, “What should Harvey call you?”, first-name input, “Let’s go →” CTA. On submit: PATCH `/api/user/name`, then redirect to `/onboarding/intro`.
- `src/app/onboarding/intro/page.tsx`
  - Intro screen (Screen 2). **Multi-slide** (3 slides), same rainbow gradient. Slide 0: problem (generic text + image cards). Slide 1 (HARVEY'S ANSWER): side-by-side — 45% glass text card, 45% glass image card with screenshot-chat.png. Slide 2 (THE DETAILS): screenshot-timeline.png right (68%), text top-left (no card), penguin bottom-left; no mockup. Bottom: progress dots + “Next →” / “I'm ready →”; last CTA → `/onboarding/questions`. User name from Supabase session.
...
- `src/app/onboarding/vision/page.tsx`
  - Vision screen. Client component. Same full-page rainbow gradient (aurora-bg) as signin. No outer card. Main content: flex column, gap 20px, padding 20px 80px, max-width 860px. Three individual glass cards (max-width 580px): TODAY (align-start), TOMORROW (align-end), THE VISION (align-start). Centered "Almost there →" button below → `/onboarding/closer`. Harvey logo at top. User name from Supabase session.
- `src/app/onboarding/closer/page.tsx`
  - Closer screen. Client component. Same layout and card styling. Eyebrow "YOU'RE EARLY", headline "[name], you're one of the first.", body, CTA "Let's build something →" → `/onboarding`. Name from session.
