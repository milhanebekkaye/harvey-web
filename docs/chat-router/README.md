...
Messages are persisted to the Discussion model (type: "project") after each exchange. The API route's `onFinish` callback saves both the user message and assistant response. For feedback buttons, `POST /api/discussions/[projectId]/messages` supports `widgetAnswer` metadata and marks the matching widget message `answered: true` in the same write as the appended response message. The last 15 messages are sent to Claude for conversation history; the system prompt covers long-term knowledge.
