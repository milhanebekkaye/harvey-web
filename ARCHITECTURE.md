...
- **`onboarding/vision/page.tsx`**: `/onboarding/vision` route. Client component. Same full-page rainbow gradient as signin. No outer card. Main content: flex column, gap 20px, padding 20px 80px, max-width 860px. Three individual glass cards (max-width 580px): Card 1 (TODAY) align-self flex-start, Card 2 (TOMORROW) align-self flex-end, Card 3 (THE VISION) align-self flex-start. Centered purple "Almost there →" button below cards → `/onboarding/closer`. Harvey logo at top. User name from Supabase session.
- **`onboarding/closer/page.tsx`**: `/onboarding/closer` route. Client component. Same layout and card styling. Eyebrow "YOU'RE EARLY", headline "[name], you're one of the first.", body, CTA "Let's build something →" → `/onboarding`. Name from session.
