...
- **`onboarding/welcome/page.tsx`**: `/onboarding/welcome` route. Uses **AuthPageLayout** + **WelcomeNameCard** (penguin avatar). Shown right after auth for new users who have no name set. Collects first name via PATCH `/api/user/name`, then sessionStorage `harvey_user_name` (trimmed), then redirects to `/onboarding/intro`. Submit in WelcomeNameCard.
- **`onboarding/intro/page.tsx`**: `/onboarding/intro` route. Screen 2: **multi-slide** client component (3 slides). Same full-page rainbow gradient. One slide visible; horizontal transition (current exits left, next from right). Top: Harvey logo (auth-style), 24px padding. **Slide 0**: Generic layout (text card + image card, problem + illustration). SlideContent accepts optional `subheadline`; when present it is rendered between headline and body (17px semibold). Slide 0 copy: headline "Sound familiar, [name]?", optional subheadline "You know exactly what you want to build. So why aren't you building it?", body about freezing and not knowing where to start. **Slide 1 (HARVEY'S ANSWER)**: Side-by-side — left 45% glass text card, right 45% glass image card with screenshot-chat.png. **Slide 2 (THE DETAILS)**: Full-bleed — screenshot-timeline.png absolute right (68%), text absolute top-left (28% width, no card), penguin (penguin-scarf.png) bottom-left; no mockup frame. Bottom: progress dots + CTA “Next →” / “I'm ready →”; slide 2 CTA redirects to `/onboarding/questions`. User name: sessionStorage `harvey_user_name` if set, else GET `/api/user/me`, else session metadata (slide 0 headline).
- **`onboarding/questions/page.tsx`**: `/onboarding/questions` route. Screen 3: six questions (reason, current work, work style, biggest challenge, coaching style, experience level). Same aurora/glass layout; 80×80 avatar, 6 progress dots, one question at a time with slide animation. On completion, PATCH `/api/user/onboarding` then redirect to `/onboarding/vision`.
- **`onboarding/vision/page.tsx`**: `/onboarding/vision` route. Client component. Same full-page rainbow gradient as signin. No outer card. Main content: flex column, gap 20px, padding 20px 80px, max-width 860px. Three individual glass cards (max-width 580px): each card has eyebrow (TODAY / TOMORROW / THE VISION), headline (20px, 800 weight, #0d101b), and body. Card 1 (TODAY) "No more blank screen." + body about clear next steps and shipping; Card 2 (TOMORROW) "Harvey learns how you actually work." + body about adaptation and learning; Card 3 (THE VISION) "The AI co-founder you never had." + body about AI partner for entrepreneurial life. Card 1 and 3 align-self flex-start, Card 2 flex-end. Centered purple "Almost there →" button below cards → `/onboarding/closer`. Harvey logo at top. User name: sessionStorage `harvey_user_name` if set, else GET `/api/user/me`, else session metadata.
- **`onboarding/closer/page.tsx`**: `/onboarding/closer` route. Client component. Same layout and card styling. Eyebrow "YOU'RE EARLY", headline "[name], you're one of the first.", body, CTA "Let's build something →" → `/onboarding`. Name: sessionStorage `harvey_user_name` if set, else GET `/api/user/me`, else session metadata.
- **`onboarding/page.tsx`**: `/onboarding` route. **Reload persistence (Batch 4)**: On mount, calls `GET /api/onboarding/restore` (optional `?projectId=` from URL). If restore returns `completed`, redirects to `/dashboard`. If restore returns `projectId` + messages, renders chat with those messages and runs extraction to fill the shadow panel; otherwise fetches **personalized greeting** via `GET /api/onboarding/greeting` (Haiku, name + profile) and shows it as Harvey’s first message (or fallback greeting on error). Prevents duplicate projects on refresh. Split layout: 40% chat (left), 60% Shadow Panel (right). No top-of-page progress bar; the only progress indicator is the completion bar inside **ProjectShadowPanel**. After each Harvey response, triggers extraction via `POST /api/onboarding/extract` with `previousConfidence` (current `harveyConfidence`); stores result in `shadowFields` and `missingBlockingFields`. Raw `harveyConfidence` drives button logic; **maxHarveyConfidence** (floor: never decreases) is passed to the panel and modal for display. Chat request body includes `currentConfidence: harveyConfidence` so Harvey only gives the recap when score ≥ 80. **Build My Schedule button**: disabled when field completeness &lt; 40% or any blocking field is missing (`missingBlockingFields`); Stage 1 when can build but confidence &lt; 80% and no completion marker; Stage 2 when can build and (confidence ≥ 80% or completion marker). **Batch 5**: Progress shows Harvey's confidence (`completion_confidence`). Button “Build My Schedule” button (disabled / Stage 1 with confirmation modal / Stage 2 direct to schedule). Button lives at bottom of right column; confirmation modal “Build now or keep chatting?” for Stage 1. Extraction is non-blocking; errors are logged only.
...
- **`dashboard/page.tsx`**: `/dashboard` route. Main authenticated user experience; layout is **DashboardSidebar** (collapsible) | **ChatSidebar** (flex-4) | main (flex-6). Resolves `userId` from Supabase `auth.getUser()` for GuidedTour. On mount, `checkTourStatus()` calls `GET /api/user/me` and sets `has_completed_tour`, `userName`, and `userPlan` for the sidebar. State: `isSidebarOpen`, `feedbackExternalOpen` (for FeedbackButton). **DashboardSidebar** handles nav, conversations (Recent chats: Project Chat with pin icon + task list), Project Details and "Vote for next features" links, user menu (Settings, Vote for next features, feedback, Upgrade Plan → Stripe payment link when free, Log out). Header uses Harvey penguin image (/harvey/penguin-hat.png). Pro Plan = active/pro/paid. **ChatSidebar** no longer has icon rail or ConversationNavPanel; conversation switching is via DashboardSidebar. **FeedbackButton** receives `externalOpen`/`onExternalOpenHandled` so the sidebar user menu can open the feedback modal. Payment success: Suspense-wrapped `PaymentSuccessHandler` detects `?payment=success`, shows toast, cleans URL. See `docs/payments/README.md` (Step 5c).
- **`dashboard/roadmap/page.tsx`**: `/dashboard/roadmap` route. **Feature Roadmap** page: lists features from GET `/api/features` (voteCount, hasVoted), vote toggle via POST `/api/features/[featureId]/vote` with optimistic UI. Empty state when no features. Same layout pattern as Settings (warm background, Back to dashboard). See `docs/feature-voting/README.md`.
- **`dashboard/settings/page.tsx`**: `/dashboard/settings` route. Full-page Settings with layout aligned to Project Details: (1) **Sticky frosted top bar**: Dashboard back link (with unsaved confirm), right = "All changes saved" or Discard + "Save Changes" purple pill; (2) **Header**: title, subtitle, user profile card (avatar, name, email, Pro/Free Plan badge from GET `/api/user/me`); (3) **Tab bar** (Schedule, Preferences, Harvey's Notes with count badge); (4) **Tab content**: Schedule = Work Schedule + Availability Windows; Preferences = editorial grid (energy, rest days, session length, communication style); Harvey's Notes = notes list with Read more/Key insight/Add Note. Data from GET `/api/settings`; save via POST `/api/settings/update`. `savedSnapshot` tracks last persisted state for hasChanges. Save/discard in top bar (no StickyUnsavedBar). See `docs/settings/README.md`.
- **`dashboard/project/[projectId]/page.tsx`**: `/dashboard/project/[projectId]` route. **Project Details page** (Feature C): view and edit project-level context (description, goals, deadline, project type, skill level, tools & stack, weekly hours, motivation, phases, Harvey’s notes). Server component fetches project via `getProjectById`; client form persists via PATCH `/api/projects/[projectId]`. Layout: sticky top bar (Dashboard back + save/discard), hero (status, title, description, metadata card with progress ring), phase stepper + tabs (Overview, Phases, Harvey’s Notes), tab content. See `docs/project-details/README.md`.
...
- **`user/onboarding/route.ts`**
  - Endpoint under `PATCH /api/user/onboarding`. Auth required. Body: `{ onboarding_reason?, current_work?, work_style?, biggest_challenge?, coaching_style?, experience_level? }`. Updates the current user’s onboarding-question fields via `updateUser`. Used by `/onboarding/questions` on completion.

- **`user/me/route.ts`**
  - Endpoint under `GET /api/user/me`. Auth required. Returns the current user profile fields needed by the frontend: `name`, `has_completed_tour`, and `payment_status` (`"free"` | `"paid"`).

- **`user/tour-complete/route.ts`**
  - Endpoint under `PATCH /api/user/tour-complete`. Auth required. Marks the current user’s dashboard guided tour as completed by setting `has_completed_tour = true`.

- **`webhooks/stripe/route.ts`**
  - Endpoint under `POST /api/webhooks/stripe`. Stripe webhook: no auth (verified via `stripe-signature` and `STRIPE_WEBHOOK_SECRET`). On `checkout.session.completed`, reads `client_reference_id` (user ID from Payment Link) and sets `User.payment_status = 'paid'`. Payment Link must include user ID as `client_reference_id`. See `docs/payments/README.md`.

- **`feedback/route.ts`**
  - Endpoint under `POST /api/feedback`. Auth required. Body: `{ label: string, content: string }`. Validates `label` in (bug, improvement, feature_request, question, other) and `content` non-empty. Loads user via `getUserById`; `userName` = DB name, else email, else "Anonymous". Creates Feedback with status "new". Returns 201 with `{ success: true }`. See `docs/feedback/README.md`.

- **`features/route.ts`**
  - Endpoint under `GET /api/features`. Auth required. Returns `{ features: [...] }` sorted by voteCount descending; each item includes id, title, description, createdAt, voteCount, and hasVoted. See `docs/feature-voting/README.md`.

- **`features/[featureId]/vote/route.ts`**
  - Endpoint under `POST /api/features/[featureId]/vote`. Auth required. Toggle vote: if user already voted, deletes vote and returns 200 `{ voted: false }`; else creates vote and returns 201 `{ voted: true }`. 404 if feature not found. See `docs/feature-voting/README.md`.
...
- **`onboarding/greeting/route.ts`**
  - Endpoint under `GET /api/onboarding/greeting`. Auth required. Fetches full user from DB (404 if not found); builds `userProfile` via `buildUserProfile`; calls Haiku (`MODELS.ONBOARDING_EXTRACTION`) with system prompt for a 2–3 sentence personalized greeting (first name, profile, one open question, coaching tone). Returns `{ greeting: string }`; on error returns fallback greeting with 200. Cost tracked via `logApiUsage` (feature `onboarding_greeting`). Used by onboarding page when there is no restore (new session).
...
- **`StickyUnsavedBar.tsx`**: Sticky bar fixed at bottom (z-[60]) shown when a form has unsaved changes. Displays “You have unsaved changes” and Discard / Save Changes buttons. Project Details and Settings use a sticky top bar for save/discard instead; other pages may still use this component.
...
- **`ChatSidebar.tsx`**: Shell for project and task conversations. Renders minimal header (discussion label only, e.g. "Project Chat" or task name; branding is in DashboardSidebar) and either **ProjectChatView** or **TaskChatView** based on `activeConversation`. Conversation switching is handled by **DashboardSidebar**; no icon rail or ConversationNavPanel. All project chat logic (useChat, messages, rebuild) lives in ProjectChatView. See **Per-task chat** below and `docs/per-task-chat.md`.
...
- **`DashboardSidebar.tsx`**: Collapsible left sidebar (Claude/Notion-style), integrated as the leftmost dashboard column. When closed: thin strip (w-12) with hamburger. When open: panel (w-64) with header (Harvey penguin image + "Harvey AI" + close), Dashboard as active nav pill, "Recent chats" (Project Chat with push_pin icon + openTaskChats with category dots), links (Project Details, "Vote for next features"), user section at bottom (avatar, userName, userPlan label — Pro Plan when active/pro/paid else Free Plan, menu: Settings, "Vote for next features", feedback, Upgrade Plan [Stripe payment link when free, else greyed], Log out). Uses `NEXT_PUBLIC_STRIPE_PAYMENT_LINK` for Upgrade when not on paid plan. See `docs/dashboard/README.md`.
...
- **`ProjectDropdownMenu.tsx`**: Dropdown menu below the project pill in the chat sidebar. Options: Project Details (link to `/dashboard/project/[projectId]`), Roadmap (link to `/dashboard/roadmap`), User Settings (link to `/dashboard/settings`), and disabled placeholders for Archive Project / Switch Project. Closes on outside click or item click.
...
- **`ProjectDetailsForm.tsx`**: Client form for the Project Details page. Layout: (1) sticky top bar with Dashboard back link and save/discard (or “All changes saved”); (2) hero with status pill, relative “Last updated”, editable title, description, “Created” date, and metadata card (progress ring + Deadline, Skill level, Stack, Commitment, Type); (3) phase stepper + tabs (Overview, Phases, Harvey’s Notes); (4) tab content (Overview = goals, motivation, stack, details, milestones; Phases = timeline CRUD; Notes = list with add/edit/remove, Read more for long notes, “Key insight” for CRITICAL). Uses **EditableField** throughout; PATCH to `/api/projects/[projectId]`; toast and unsaved-changes guard (beforeunload + confirm on navigation). StickyUnsavedBar is not used here (save/discard in top bar).
...
- **`GuidedTour.tsx`**: Self-contained spotlight overlay tour (4 spotlight steps + paywall step 5). Uses `document.querySelector('[data-tour="..."]')` to locate targets. Spotlight: fixed cutout with purple glow; tooltip card position-computed per step. Dashboard reads `GET /api/user/me` on mount; if `has_completed_tour === false` and `userId` is available (from Supabase), renders the tour with `userId` prop. Paywall "Become a Founding Member" opens Stripe Payment Link with `?client_reference_id=<userId>` in a new tab, scrolls to top, then calls `onComplete()`; "I'll join later" scrolls and completes. Paywall copy: headline "You're in the founding cohort.", subheadline on $20 founding price and shaping Harvey; feature lines (unlimited coaching/scheduling/rescheduling, Harvey learns patterns, per-task conversations). On completion, fires `PATCH /api/user/tour-complete` (fire-and-forget). See `docs/guided-tour/README.md` and `docs/payments/README.md`.
- **`FeedbackButton.tsx`**: Floating button (fixed bottom-right, purple pill) "What would make Harvey better?" Opens a modal to submit feedback: label chips (Bug, Improvement, Feature Request, Question, Other), textarea, Submit → POST `/api/feedback`. Success state 2s then close. Backdrop click and Escape close modal. Optional props: `externalOpen` (when true, opens the modal) and `onExternalOpenHandled` (callback so parent can reset state after opening); enables opening from e.g. DashboardSidebar user menu. See `docs/feedback/README.md`.
...
- **`WelcomeNameCard.tsx`**: Card content for the **onboarding welcome page only** (`/onboarding/welcome`): penguin avatar (160×160), “What should Harvey call you?”, first-name input, “Let’s go →” CTA. On submit: PATCH `/api/user/name`, then sessionStorage.setItem('harvey_user_name', trimmed), then redirect to `/onboarding/intro`. Used inside AuthPageLayout. Logo/avatar is independent of the signin page (SigninCard uses the small Harvey sparkle icon).
...
- **`usage-logger.ts`**: Async `logApiUsage({ userId, feature, model, inputTokens, outputTokens })`. Writes to `ApiUsageLog` and upserts `UserUsageSummary` (30-day period from user `subscription_start_date` or `createdAt`). Never throws; failures are logged only. **Wired** into all Anthropic call sites: non-streaming (Phase 3) and all four streaming routes (Phase 4 — usage from `result.usage` in onFinish or when stream completes). **New API calls must call `logApiUsage`** — see `docs/cost-tracking/README.md`.
- **`prompts.ts`**: All prompt templates and system instructions for AI interactions. **Onboarding**: `ONBOARDING_SYSTEM_PROMPT(todayFormatted, knownInfo, missingFieldsGuidance, userProfile, currentConfidence, todayISO?, tomorrowISO?)` returns the system prompt with unambiguous date at the top ("TODAY IS: ..."), known-info summary, optional USER PROFILE block (from `buildUserProfile(user)` — six onboarding-question fields), and dynamic missing-fields guidance; `generateKnownInfoSummary(projectData, userData)` builds the summary of already-extracted fields so Harvey doesn’t re-ask; `buildUserProfile(user)` formats onboarding_reason, current_work, work_style, biggest_challenge, coaching_style, experience_level (non-null only). `COMPLETION_MARKER` is still used for intake-complete detection.
...
- **User** (life constraints and availability, shared across all projects): `workSchedule`, `commute`, `timezone`, `preferred_session_length`, `communication_style`, **`energy_peak`**, **`availabilityWindows`** (when the user can work; single source for scheduling), **`rest_days`** (days user doesn't want to work), **`oneOffBlocks`** (date-specific unavailability). Blocked time is derived from workSchedule and commute when building the availability map.
- **Project** (enrichment and schedule): **schedule_start_date** (DateTime?, optional first day of schedule), **schedule_duration_days** (how many days the schedule spans; preferred over weeks), **exclusions** (feature exclusions for task generation, e.g. ["messaging", "payment integration"]). **Project.contextData** is deprecated and no longer read or written for scheduling; all scheduling data lives on User and Project first-class fields.
...
- **`schema.prisma`**: Source of truth for the database schema (models such as User, Project, Task, Schedule, Discussion, etc.). Changes here are applied to the DB via migrations. The **User** model includes `timezone`, **life constraints** (`workSchedule` Json, `commute` Json, `availabilityWindows` Json with **fixed** / **flexible** window types), enrichment (`preferred_session_length`, `communication_style`, `userNotes Json?`), **`has_completed_tour Boolean @default(false)`** for the one-time dashboard guided tour, and **`payment_status String @default("free")`** (`"free"` | `"paid"`) for Stripe billing. The **Project** model includes **`exclusions String[]`** (feature exclusions for task generation), **`contextData Json?`** (deprecated; no longer used for scheduling), enrichment fields (`target_deadline`, `skill_level`, etc.), `projectNotes Json?`, `generationCount Int`, **`milestones Json?`** (array of milestone objects from schedule generation; displayed on Project Details page), **`schedule_duration_days Int?`** (calendar days the schedule spans; TODO: move to Schedule/Batch model when Feature 8 multi-generation ships), and **`schedule_start_date DateTime?`** (optional first day of schedule; generate-schedule uses it when set, else defaults to tomorrow/next Monday). The **Task** model includes **`position Int?`** (per-day sort order, 1-based; null = legacy, sort by scheduledStartTime), `depends_on String[]`, `batchNumber Int`, for flexible-scheduled tasks: **`window_start`**, **`window_end`** (String? — store **availability window boundaries** e.g. "09:00"/"17:00", not computed slot times), `is_flexible` (Boolean), and (Session 4) **`energy_required`** (String?: "high"|"medium"|"low"), **`preferred_slot`** (String?: "peak_energy"|"normal"|"flexible"). **Sort order** for task lists: `scheduledDate` → `position` → `priority`. The **Discussion** model includes `type String` (default "project"; values "onboarding" | "project" | "task"), `taskId String?`, and optional **`task Task?`** relation (onDelete: Cascade). The **Task** model has **`discussions Discussion[]`** (inverse). Per-task chat (Step 2) uses type "task" and taskId to store one discussion per task. The **User** model has **`feedbacks Feedback[]`** and **`featureVotes FeatureVote[]`** for the feedback system and feature-voting board. **Feedback** stores user-submitted feedback (label: bug | improvement | feature_request | question | other; status: new | seen | resolved). **Feature** holds proposed features (title, description) for the voting board. **FeatureVote** is a unique user vote per feature (unique on featureId + userId). See `docs/feedback/README.md` and `docs/feature-voting/README.md`.

- **`migrations/`**: Auto-generated migration history:
  - **`20260307180000_add_feedback_and_feature_voting/`**
    - **`migration.sql`**: Creates tables `feedbacks`, `features`, `feature_votes`; adds User relations for feedback and feature voting.
