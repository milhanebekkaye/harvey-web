...
- **`src/prisma/`**: Prisma schema and migrations (see `schema.prisma`). Core models: User, Project, Discussion, Task. **API cost tracking**: User has optional `subscription_start_date`; `ApiUsageLog` stores per-call token/cost; `UserUsageSummary` stores per-user, per–billing-period aggregates (unique on userId + periodStart). See `docs/cost-audit.md`. **When adding a new Anthropic API call**, you must log usage via `logApiUsage` — see `docs/cost-tracking/README.md` for the pattern.
...
- **`user/me/route.ts`**
  - Endpoint under `GET /api/user/me`. Auth required. Returns the current user profile fields currently needed by the frontend: `name` and `has_completed_tour`.
...
- **`GuidedTour.tsx`**: Self-contained spotlight overlay tour (3 steps) shown once after a user's first schedule generation. Uses `document.querySelector('[data-tour="..."]')` to locate targets set in Step 2. Spotlight effect: fixed `div` with `boxShadow: '0 0 0 9999px rgba(0,0,0,0.6)'` creates a cutout without modifying any element's own styles. A full-screen blocker (`z-index: 59`) prevents page interaction during the tour; tooltip card (`z-index: 61`) is position-computed per step (`left` | `right` | `top`) with viewport boundary clamping. Dashboard reads `GET /api/user/me` on mount; if `has_completed_tour === false`, renders the tour. On completion, fires `PATCH /api/user/tour-complete` (fire-and-forget). See `docs/guided-tour/README.md`.
