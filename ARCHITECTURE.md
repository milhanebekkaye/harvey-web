...
- **`dashboard/page.tsx`**: `/dashboard` route. Main authenticated user experience; shows tasks, timeline, calendar, and chat sidebar. Resolves `userId` from Supabase `auth.getUser()` for GuidedTour. On URL `?payment=success` (redirect from Stripe), shows a success toast at top of right panel and cleans URL; toast auto-dismisses after 5s. See `docs/payments/README.md` (Step 5c).
