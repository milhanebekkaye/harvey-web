...
- **`ChatSidebar.tsx`**: Shell for project and task conversations. Renders header (Harvey AI + discussion label) and either **ProjectChatView** or **TaskChatView** based on `activeConversation`. Conversation switching is handled by **DashboardSidebar**; no icon rail or ConversationNavPanel. All project chat logic (useChat, messages, rebuild) lives in ProjectChatView. See **Per-task chat** below and `docs/per-task-chat.md`.
...
- **`DashboardSidebar.tsx`**: Collapsible left sidebar (Claude/Notion-style), integrated as the leftmost dashboard column. When closed: thin strip (w-12) with hamburger to open. When open: panel (w-64) with header (Harvey AI + close), Dashboard as active nav pill, Conversations (Project Chat + openTaskChats with category dots), links (Project Details, Roadmap), and user section at bottom (avatar, userName, userPlan, menu above with Settings, Roadmap, feedback trigger, Upgrade Plan, Log out). Props: isOpen, onToggle, openTaskChats, activeConversation, onSelectConversation, projectId, projectTitle, userName, userPlan, onSignOut, onOpenFeedback. See `docs/dashboard/README.md`.
