...
- **`dashboard/page.tsx`**: `/dashboard` route. Main authenticated user experience; shows tasks, timeline, calendar, and chat sidebar. Resolves `userId` from Supabase `auth.getUser()` for GuidedTour. Payment success: a Suspense-wrapped `PaymentSuccessHandler` (same file) uses `useSearchParams()` to detect `?payment=success`, invokes parent callback to show success toast and clean URL; toast auto-dismisses after 5s. Renders **FeedbackButton** (floating bottom-right) for the feedback modal. See `docs/payments/README.md` (Step 5c).
...
- **`FeedbackButton.tsx`**: Floating button (fixed bottom-right, purple pill) "What would make Harvey better?" Opens a modal to submit feedback: label chips (Bug, Improvement, Feature Request, Question, Other), textarea, Submit → POST `/api/feedback`. Success state 2s then close. Backdrop click and Escape close modal. See `docs/feedback/README.md`.
