...
- **`chat/route.ts`**
  ...
  - **Onboarding prompt**: When `context === 'onboarding'`, the system prompt is built by `ONBOARDING_SYSTEM_PROMPT(todayFormatted, knownInfo, missingFieldsGuidance)`. **Date**: `todayFormatted` is a long unambiguous date at the very top (e.g. "Monday, February 17, 2026. All scheduling starts from today.") so Harvey computes "in 6 days" / "next Friday" correctly. **Known info**: `generateKnownInfoSummary(project, user)` when `projectId` is present; first message uses "Starting fresh". **Missing fields**: When `projectId` exists, `computeMissingFields(projectId, userId)` and `buildMissingFieldsGuidance(blocking, enriching)` from `src/lib/onboarding/missing-fields.ts` inject dynamic guidance so Harvey asks for critical (blocking) and helpful (enriching) fields naturally.
...
- **`onboarding/extract/route.ts`**
  ...
  - **Feature D (Shadow Panel) Step 2 + 3, Batch 5**: Extraction + persistence. POST body: `{ projectId }`. Authenticates user, verifies project ownership, loads onboarding discussion via `getOnboardingDiscussion`, builds full conversation text, calls Anthropic Haiku with a structured extraction prompt that includes **confidence assessment**: model returns `completion_confidence` (0–100) reflecting depth of understanding. Parses and validates JSON; validates/clamps `completion_confidence` (default 0 if missing). **Merge logic**: only non-null extracted fields are written (no overwriting with null). Uses `updateUser` and `updateProject` to persist. **After save**: calls `computeMissingFields(projectId, userId)` from `src/lib/onboarding/missing-fields.ts` and returns `missingBlockingFields` and `missingEnrichingFields` so the frontend can gate the "Build my schedule" button (disabled until no blocking fields missing) and Harvey receives dynamic guidance on the next chat turn. Response: `{ success: true, extracted: { user, project }, saved: { user, project }, completion_confidence, missingBlockingFields, missingEnrichingFields }`. See `docs/onboarding/README.md`.
...
- **`schedule/generate-schedule/route.ts`**
  ...
  - Relies on `src/lib/schedule/schedule-generation.ts` and `src/lib/schedule/task-scheduler.ts`; logs SchedulerOptions and task records for debugging.
