...
- **`onboarding/welcome/page.tsx`**: `/onboarding/welcome` route. Uses **AuthPageLayout** + **WelcomeNameCard** (penguin avatar). Shown right after auth for new users who have no name set. Collects first name via PATCH `/api/user/name`, then redirects to `/onboarding/questions`.
